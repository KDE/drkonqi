...
#warning could use more descriptive messaging in both cases. maybe tell the user to upgrade
        if (mapping->bugzillaProductDisabled()) {
            ui->errorLabel->setText(xi18nc("@info", "The Product <resource>%1</resource> is no longer supported.", mapping->bugzillaProduct()));
            errorOut();
            return;
        }
