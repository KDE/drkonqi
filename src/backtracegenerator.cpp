...
void BacktraceGenerator::slotProcessExited(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (m_sentryPayload.isEmpty()) {
        m_state = PreambleFailed;
        Q_EMIT stateChanged();
        Q_EMIT someError();
        return;
    }
...
void BacktraceGenerator::startProcess()
{
    ...
    s_fence.surroundMe();
    connect(&s_fence, &MemoryFence::active, this, &BacktraceGenerator::startProcessInternal);
}
...
{
    return m_state == State::Failed || m_state == State::FailedToStart || m_state == State::PreambleFailed || m_state == State::MemoryPressure;
}
